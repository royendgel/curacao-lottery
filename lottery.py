...
class Lottery(object):
    def __init__(self, only_new=True, database='database/basedb.db', new=False):
        self.range_year = 0
        self.range_month = 0
        self.database = database
        self.connection = sqlite3.connect(self.database)
        # if only_new == True:
        #     self.get_range(start_year=2014, end_year=2014, start_month=1, end_month=2)
    ...
    def get_number_by_year(self, year, number='0000'):
        d = self.get_range(year, year, 1, 12, number)
        return self.get_number_by_something(number, d)
    ...
    def save_extracted_data(self, data):
        ...
        for drawing_list in data:
            for d in drawing_list:
                ...
                self.connection.cursor().execute("INSERT INTO Drawing(Date, Position, Drawing) VALUES(?, ?, ?)",
                                                 draw_data)
    ...
    def generate_not_drawn(self):
        print(
        'Processing numbers that has not been drawn....')
...
if __name__ == '__main__':
    ...
    lot = Lottery()
    result = lot.search_number(number=x)
    print(result)
